...
{
    /// <summary>
    /// The signature_algorithms GREASE value. Zero when the caller did not enable
    /// signature_algorithms GREASE; the encoder gates on the configuration flag, not on this.
    /// </summary>
    internal ushort SignatureAlgorithm { get; init; }
    ...
    internal ushort Get(ClientHelloGreaseSlot slot) => slot switch
    {
        ...
        ClientHelloGreaseSlot.SignatureAlgorithm => SignatureAlgorithm,
...
internal static class ClientHelloEncoder
{
        ...
        try
        {
            ...
            ClientHelloGreaseValues? greaseValues = configuration.GreasePolicy is { } greasePolicy
                ? retry?.GreaseValues ?? SelectGreaseValues(
                    ...
                    configuration.GreaseSignatureAlgorithms,
                    randomSource)
                : null;
        ...
        foreach (var extension in order)
        {
            ...
            result.Add(kind switch
            {
                ...
                ClientHelloExtensionKind.SignatureAlgorithms => EncodeSignatureAlgorithms(
                    ...
                    configuration.GreaseSignatureAlgorithms && greaseValues.HasValue
                        ? greaseValues.Value.SignatureAlgorithm
                        : null,
                    configuration.GreaseSignatureAlgorithmIndex),
    ...
    private static EncodedExtension EncodeSignatureAlgorithms(
        ...
        ushort? greaseValue = null,
        int greaseIndex = 0)
    {
        // BoringSSL prepends the fake sigalg (`ssl/extensions.cc` @ main L1031-1036, before
        // tls12_add_verify_sigalgs), which is why index 0 is the caller default. The index is
        // a knob because no source bounds where a non-BoringSSL client would put it.
        var insertAt = greaseValue.HasValue
            ? Math.Clamp(greaseIndex, 0, algorithms.Count)
            : -1;
        var values = new TlsBinaryWriter();
        for (var index = 0; index <= algorithms.Count; index++)
        {
            ...
            {
                values.WriteUInt16((ushort)algorithms[index]);
            }
        }
        var data = new TlsBinaryWriter();
        data.WriteVector16(values.WrittenSpan);
    ...
    private static readonly ClientHelloGreaseSlot[] AlwaysGeneratedGreaseSlots =
    [
        ClientHelloGreaseSlot.CipherSuite,
        ClientHelloGreaseSlot.SupportedVersion,
        ClientHelloGreaseSlot.SupportedGroup,
        ClientHelloGreaseSlot.KeyShare,
        ClientHelloGreaseSlot.Extension,
    ];

    private static ClientHelloGreaseValues SelectGreaseValues(
        ClientHelloGreasePolicy policy,
        bool includeSecondaryExtension,
        bool includeSignatureAlgorithm,
        IRandomSource randomSource)
    {
        // Only slots that are actually emitted draw a value, so enabling one optional slot
        // never perturbs the values another slot would have received.
        var active = new List<ClientHelloGreaseSlot>(AlwaysGeneratedGreaseSlots);
        if (includeSecondaryExtension)
        {
            active.Add(ClientHelloGreaseSlot.SecondaryExtension);
        }
        if (includeSignatureAlgorithm)
        {
            active.Add(ClientHelloGreaseSlot.SignatureAlgorithm);
        }

        var generatedCount = policy.GetGeneratedValueCount(active.ToArray());
        ...
        return new ClientHelloGreaseValues(
            ...
            includeSecondaryExtension
                ...
                : generated[policy.GetValueClass(ClientHelloGreaseSlot.Extension)])
        {
            SignatureAlgorithm = includeSignatureAlgorithm
                ? generated[policy.GetValueClass(ClientHelloGreaseSlot.SignatureAlgorithm)]
                : (ushort)0,
        };
    }
