...
public sealed class ClientHelloBuilder
{
    ...
    private bool _greaseSignatureAlgorithms;
    private int _greaseSignatureAlgorithmIndex;
    ...
    /// <summary>Sets TLS CertificateVerify signature schemes in exact wire order.</summary>
    ...
    /// <summary>
    /// Splices one RFC 8701 GREASE value into signature_algorithms, as Chromium does by
    /// default since 2026-06-30 (`net/base/features.cc` @ main L1004,
    /// <c>BASE_FEATURE(kTlsGreaseSigalgs, base::FEATURE_ENABLED_BY_DEFAULT)</c>, reaching
    /// BoringSSL through <c>SSL_CTX_set_grease_sigalgs_enabled</c> at
    /// `net/socket/ssl_client_socket_impl.cc` @ main L211-212).
    /// </summary>
    /// <param name="enabled">Whether to emit the value. Older clients emit none.</param>
    /// <param name="index">
    /// Where the value lands in the list. BoringSSL prepends it (`ssl/extensions.cc` @ main
    /// L1031-1036), so 0 is the default; the index is clamped into range. Whether any real
    /// client places it elsewhere is UNVERIFIED — a capture of a non-BoringSSL client that
    /// greases signature_algorithms would settle it.
    /// </param>
    /// <remarks>
    /// Has no effect unless GREASE is enabled via <see cref="WithGrease(bool)"/> or
    /// <see cref="WithGrease(ClientHelloGreasePolicy)"/>; it never throws on its own.
    /// The emitted value's sharing with other placements is set by
    /// <see cref="ClientHelloGreasePolicy.CreateWithSignatureAlgorithm"/>.
    /// </remarks>
    public ClientHelloBuilder WithGreaseSignatureAlgorithms(bool enabled = true, int index = 0)
    {
        _greaseSignatureAlgorithms = enabled;
        _greaseSignatureAlgorithmIndex = index;
        return this;
    }
    ...
    {
        // Duplicate-rejection below is NOT uniform, because the specifications are not.
        // Each call is annotated with the text that does or does not back it; see
        // ValidateUnique for why that distinction is load-bearing in a fidelity library.

        // OUR RULE, not the RFC's. RFC 9846 §4.2.2 (RFC 8446 §4.1.2) constrains only the
        // order — "A list of the symmetric cipher options supported by the client ... in
        // descending order of client preference" — and says nothing about repeats.
        ValidateUniqueAndNonEmpty(_cipherSuites, nameof(_cipherSuites));

        // OUR RULE, not the RFC's. RFC 9846 §4.3.1 (RFC 8446 §4.2.1): "The extension
        // contains a list of supported versions in preference order, with the most
        // preferred version first." No duplicate rule anywhere in the section.
        ValidateUniqueAndNonEmpty(_supportedVersions, nameof(_supportedVersions));

        // SPEC-BACKED. RFC 9846 §4.3.7: "The "named_group_list" MUST NOT contain any
        // duplicate entries. A recipient MAY abort a connection with a fatal
        // "illegal_parameter" alert if it detects a duplicate entry." (New in the bis;
        // RFC 8446 §4.2.7 had no such rule.) Do not relax without new evidence.
        ...
        // OUR RULE, not the RFC's, and opt-out for exactly that reason. RFC 9846 §4.3.3
        // (RFC 8446 §4.2.3) constrains only the order — "Each SignatureScheme value lists a
        // single signature algorithm that the client is willing to verify. The values are
        // indicated in descending order of preference." — and never forbids a repeat. Real
        // devices do repeat one: the pinned Apple capture sends 0x0805 twice, and a reported
        // handset sends a ten-entry list with the same repeat in 80/80 captured connections.
        // Rejecting that would make the library unable to express bytes that demonstrably
        // exist, so the guard stays on by default only as a caller-typo net.
        ...
        var keyShareGroups = _includeKeyShareExtension
            ...
            : [];
        // SPEC-BACKED, and the strongest of the set. RFC 9846 §4.3.8 (RFC 8446 §4.2.8):
        // "Clients MUST NOT offer multiple KeyShareEntry values for the same group. ...
        // Servers MAY check for violations of these rules and abort the handshake with an
        // "illegal_parameter" alert if one is violated." Do not relax by symmetry with
        // signature_algorithms: a repeat here is a protocol violation, not a fingerprint.
        ...
        {
            // OUR RULE, not the RFC's. signature_algorithms_cert shares SignatureSchemeList
            // and inherits only the ordering sentence of RFC 9846 §4.3.3; its sole normative
            // line is "Implementations which have the same policy in both cases MAY omit the
            // "signature_algorithms_cert" extension." No evidenced client repeats an entry
            // here yet, so this stays closed until one does.
        ...
        {
            // OUR RULE, not the RFC's. RFC 9345 §4.1.1 only says "The body of the extension
            // consists of a SignatureSchemeList (defined in [RFC8446])"; the word "duplicate"
            // does not appear in RFC 9345. Closed until a capture shows otherwise.
            ValidateUniqueAndNonEmpty(
        ...
        return new ClientHelloConfiguration(
            ...
            extensionLayout)
        {
            GreaseSignatureAlgorithms = _greaseSignatureAlgorithms,
            GreaseSignatureAlgorithmIndex = _greaseSignatureAlgorithmIndex,
        };
    }
    ...
    /// <summary>
    /// Rejects a repeated entry. Only two callers are backed by normative text —
    /// key_share (RFC 9846 §4.3.8 "Clients MUST NOT offer multiple KeyShareEntry values for
    /// the same group.") and supported_groups (RFC 9846 §4.3.7 "The "named_group_list" MUST
    /// NOT contain any duplicate entries.") — and both bans are only MAY-enforced by the
    /// receiver. Everywhere else a repeat is legal-but-unusual, so it is fingerprint signal
    /// rather than a protocol error, and the guard is this library's assumption about caller
    /// intent. Before adding a caller, decide which of those two it is.
    /// </summary>
...
{
    /// <summary>Whether one GREASE value is spliced into signature_algorithms.</summary>
    internal bool GreaseSignatureAlgorithms { get; init; }

    /// <summary>Where that GREASE value lands. Clamped into range by the encoder.</summary>
    internal int GreaseSignatureAlgorithmIndex { get; init; }
    ...
    {
        GreaseSignatureAlgorithms = GreaseSignatureAlgorithms,
        GreaseSignatureAlgorithmIndex = GreaseSignatureAlgorithmIndex,
    };
}
