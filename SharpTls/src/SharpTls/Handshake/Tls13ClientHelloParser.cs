...
internal static class Tls13ClientHelloParser
{
    ...
    private static SignatureScheme[] ParseSignatureAlgorithms(ReadOnlySpan<byte> data)
    {
        ...
        // Duplicate entries are accepted. RFC 8446 section 4.2.3 declares the extension body as a
        // plain vector — "SignatureScheme supported_signature_algorithms<2..2^16-2>;" — and says
        // only that "Each SignatureScheme value lists a single signature algorithm that the client
        // is willing to verify. The values are indicated in descending order of preference." It
        // imposes no uniqueness requirement. The one duplicate prohibition in section 4.2 is about
        // extensions, not list entries: "There MUST NOT be more than one extension of the same type
        // in a given extension block."
        //
        // Rejecting a repeat is therefore stricter than the specification, and stricter than
        // reality: a passive non-MITM capture of iOS 16.7.16 / Spotify 9.1.72.1896 sends
        // 0403 0804 0401 0503 0203 0805 0805 0501 0806 0601 0201, with rsa_pss_rsae_sha384 (0x0805)
        // twice, in every one of its 87 control-plane hellos. A parser that refused it would refuse
        // a genuine, widely deployed Apple client.
