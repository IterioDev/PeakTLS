...
public sealed class CustomTlsQuicClient : IAsyncDisposable
{
    ...
    // RFC 9000 s18.2's ack_delay_exponent (0x0A) EXACTLY AS THIS CLIENT ADVERTISES IT, so that
    // a caller scaling its own ACK delays can check the two agree. Null means the ClientHello
    // carries no such parameter, in which case s18.2 applies: "if this value is absent, a
    // default value of 3 is assumed (indicating a multiplier of 8)". Reading it here rather
    // than copying it onto a spec is deliberate - TlsQuicConnectionSpec.AckRangeLimit's remarks
    // say why: "a second copy on the spec would be a second source of truth for one advertised
    // value". This is the accessor that makes the ONE source readable.
    internal ulong? AdvertisedAckDelayExponent =>
        _configuration.ClientHello.Spec.QuicTransportParameters
            ?.Get((ulong)TlsQuicTransportParameterId.AckDelayExponent)
            ?.GetVariableInteger();

    // RFC 9000 s18.2's max_idle_timeout (0x01) EXACTLY AS THIS CLIENT ADVERTISES IT, in the
    // milliseconds s18.2 states it in: "The maximum idle timeout is a value in milliseconds
    // that is encoded as an integer". Read here for the same reason as
    ...
    // RFC 9221 s3's max_datagram_frame_size (0x20) EXACTLY AS THIS CLIENT ADVERTISES IT, read
    // here for the same reason as the two above: the ClientHelloProfile is the one place a
    // transport parameter is written, so this accessor is the only honest way to ask what went
    // on the wire. THE SPEC OBJECT IS NOT THAT PLACE - TlsQuicConnectionSpec.TransportParameters
    ...
    //
    // NULL AND ZERO ARE ONE STATE, per s3: "An endpoint MUST NOT send DATAGRAM frames until it
    // has received the max_datagram_frame_size transport parameter with a non-zero value
    // during the handshake" - so an absent parameter and a zero one both mean no datagram
    // support, and callers get the same answer for both rather than having to know that.
    internal ulong? AdvertisedMaxDatagramFrameSize =>
        _configuration.ClientHello.Spec.QuicTransportParameters
            ?.Get((ulong)TlsQuicTransportParameterId.MaxDatagramFrameSize)
            ?.GetVariableInteger() is { } size and > 0 ? size : null;
