...
public sealed class CustomTlsQuicClientOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>ONE PROFILE IS ONE CONNECTION, and this property cannot enforce that. A
    /// <see cref="ClientHelloProfile"/> is immutable and carries extension 57's body as bytes
    /// fixed when it was built, so an options object reused for a second
    /// <see cref="CustomTlsQuicClient"/> re-sends that first connection's
    /// <c>initial_source_connection_id</c> - which will not be the connection ID the second
    /// connection's packets carry - together with one frozen draw of every per-connection
    /// transport parameter. Nothing here can detect either: this type never sees a connection
    /// ID, and one encoded blob looks exactly like another.</para>
    /// <para>SO BUILD IT PER CONNECTION. <c>TlsQuicClientHelloProfileFactory.Create</c> takes
    /// the source connection ID and composes the parameters afresh, and
    /// <c>TlsQuicConnection</c>'s client factory - <c>Func&lt;ReadOnlyMemory&lt;byte&gt;,
    /// CustomTlsQuicClient&gt;</c> - is called with that ID after the connection has drawn it,
    /// which is the seam that exists to make this the easy path rather than the careful
    /// one.</para>
    /// </remarks>
