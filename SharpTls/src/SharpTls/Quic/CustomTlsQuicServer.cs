...
/// </summary>
/// <remarks>
/// <para>THE CALLER ALSO OWNS RFC 9000 §7.3's TWO MANDATORY CONNECTION ID PARAMETERS, AND A
/// CONFIGURATION THAT OMITS THEM PRODUCES A SERVER NO CONFORMING CLIENT WILL COMPLETE WITH -
/// including <see cref="TlsQuicConnection"/>. §7.3: <i>"Each endpoint includes the value of the
/// Source Connection ID field from the first Initial packet it sent in the
/// initial_source_connection_id transport parameter"</i>, and <i>"A server includes the
/// Destination Connection ID field from the first Initial packet it received from the client in
/// the original_destination_connection_id transport parameter"</i>; and <i>"An endpoint MUST
/// treat the absence of the initial_source_connection_id transport parameter from either
/// endpoint or the absence of the original_destination_connection_id transport parameter from
/// the server as a connection error of type TRANSPORT_PARAMETER_ERROR."</i></para>
/// <para>BOTH VALUES ARE PER-CONNECTION AND THIS TYPE CANNOT KNOW EITHER.
/// <c>original_destination_connection_id</c> is the Destination Connection ID off the client's
/// first Initial packet, which exists only at runtime and only in the packet layer the summary
/// above already assigns to the caller; this type is handed one immutable
/// <c>CustomTlsQuicServerConfiguration</c> and copies its <c>TransportParameters</c> to the wire
/// verbatim. So the caller must build a configuration PER CONNECTION with both parameters set
/// from the connection IDs it drew and received. <c>LoopbackQuicPeer</c>'s server helper in the
/// test tree is the worked example.</para>
/// <para>NOT VALIDATED HERE, AND THAT IS THE DEFERRAL RATHER THAN AN OVERSIGHT. Making this a
/// throw would need a server-side QUIC connection driver to have somewhere to throw FROM - the
/// component that owns the connection IDs and merges them in - and A4 builds no such driver;
/// A4 is a client. Recorded as a named deferral in the A4 plan against <b>task 14</b>, which is
/// A4's last task and therefore the last chance to leave the surface honest. Task 10's MsQuic
/// gate and task 13's live run both talk to FOREIGN servers, so neither will ever notice.</para>
/// </remarks>
