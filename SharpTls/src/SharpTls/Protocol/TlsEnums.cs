...
public enum SignatureScheme : ushort
{
    ...
    // ML-DSA (FIPS 204) verify preferences. Advertise-only: SharpTls never produces an
    // ML-DSA signature, and no public CA issues ML-DSA certificates today, so offering
    // these cannot complete a handshake that selects them. They exist because Chromium
    // `main` leads its signature_algorithms list with them.
    //
    // Codepoints read from BoringSSL `include/openssl/ssl.h` @ main, L1174-L1176:
    //   #define SSL_SIGN_ML_DSA_44 0x0904
    //   #define SSL_SIGN_ML_DSA_65 0x0905
    //   #define SSL_SIGN_ML_DSA_87 0x0906
    // Sent by Chromium `net/socket/ssl_client_socket_impl.cc` @ main, `kVerifyPrefs`
    // L797-L809, which lists SSL_SIGN_ML_DSA_44/65/87 as its first three entries. The
    // `kTlsMldsaSignatures` feature flag was removed on 2026-07-30 ("Remove
    // kTlsMldsaSignatures feature flag that is on by default"), i.e. it shipped on.

    /// <summary>ML-DSA-44 (FIPS 204). Advertise-only; SharpTls cannot sign or verify it.</summary>
    MlDsa44 = 0x0904,

    /// <summary>ML-DSA-65 (FIPS 204). Advertise-only; SharpTls cannot sign or verify it.</summary>
    MlDsa65 = 0x0905,

    /// <summary>ML-DSA-87 (FIPS 204). Advertise-only; SharpTls cannot sign or verify it.</summary>
    MlDsa87 = 0x0906,
}
