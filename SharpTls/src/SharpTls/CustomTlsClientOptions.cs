...
public sealed class CustomTlsClientOptions
{
    ...
    /// <summary>
    /// Gets or sets whether a TLS 1.2 full handshake is aborted when the server does not
    /// echo <c>extended_master_secret</c>. Defaults to <see langword="false"/>, which is
    /// what widely deployed stacks — OkHttp on Conscrypt among them — actually do, and
    /// RFC 7627 section 5.3 permits a client that does not require the extension to
    /// continue with the legacy master secret.
    /// <para>
    /// Continuing costs the protection RFC 7627 exists for: without the extended master
    /// secret a full handshake is exposed to the triple-handshake attack. Set this to
    /// <see langword="true"/> for any connection where fidelity to a specific client is
    /// not the goal.
    /// </para>
    /// <para>
    /// This governs the full handshake only. A resumption whose extension state does not
    /// match the session being resumed is always fatal, as RFC 7627 section 5.3 requires.
    /// </para>
    /// </summary>
    public bool RequireExtendedMasterSecret { get; set; }
