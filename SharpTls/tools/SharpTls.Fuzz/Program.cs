...
// quic-packets-only: prints how many inputs actually reached a `true` return
// from each parser it drives, not just how many ran without throwing - a
// clean "inputs=200013" headline cannot on its own distinguish a
// seed-anchored campaign from one where mutation never got past the first
// gate. Null (no output) for every other target, which this leaves
// untouched.
var quicPacketsReachability = targets.DescribeQuicPacketsReachability();
if (quicPacketsReachability is not null)
{
    Console.WriteLine(quicPacketsReachability);
}

// Same contract for quic-frames, and the same reason: the deliverable of that
// campaign is the per-parser, per-frame-type, per-axis breakdown, not the
// iteration count above.
var quicFramesReachability = targets.DescribeQuicFramesReachability();
if (quicFramesReachability is not null)
{
    Console.WriteLine(quicFramesReachability);
}

// Same contract again for the two targets this task adds. QPACK's report is the
// deliverable of its campaign in the strictest sense: the arms it names are the
// ones a corpus of noise cannot reach on its own, so a run that printed only
// "inputs=N" could not tell a live target from decoration.
var qpackReachability = targets.DescribeQpackReachability();
if (qpackReachability is not null)
{
    Console.WriteLine(qpackReachability);
}

var http3Reachability = targets.DescribeHttp3Reachability();
if (http3Reachability is not null)
{
    Console.WriteLine(http3Reachability);
}
