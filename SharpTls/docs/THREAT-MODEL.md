...
| TM-15 | Build/package tampering or dependency substitution | Compiler output is deterministic/path-mapped; public API has a reviewed baseline; normalized NuGet/symbol packages are byte reproducible and clean-restored. CI actions are commit-SHA pinned, tag artifacts require platform, quality and public-interop jobs, and OIDC provenance attests release artifacts. **The runtime package no longer has zero third-party dependencies.** It has exactly one: `ZstdSharp.Port` (MIT), pinned to the exact range `[0.8.8]`. It exists because the shipped Firefox 148 profile advertises RFC 8879 certificate compression algorithm 3 (zstd) and .NET ships no zstd decompressor, so the alternatives were an advertisement that kills every handshake a server takes up, or an in-repo zstd decoder run on peer-controlled bytes. Supply-chain handling: the package is pure managed with no native asset and no transitive package on `net9.0`, so it adds one reviewable managed assembly and leaves the "pure managed" claim intact; the exact-version pin stops a restore floating onto an unreviewed build; the licence is recorded in THIRD-PARTY-NOTICES.md; and the reproducible-package and clean-restore jobs still cover the packed output. Its decompressor is treated as hostile-input code by `CompressedCertificateParser`, which sizes the output buffer from the peer's own declared `uncompressed_length` before decoding and maps every non-fatal exception to `bad_certificate`. **Residual risk, accepted deliberately:** a compromise of that package's publishing account is now inside the runtime trust boundary, which the previous zero-dependency posture excluded by construction; this mitigation is strictly weaker than the sentence it replaces. No other runtime dependency may be added without amending this row. Hosted run/attestation IDs remain external evidence. |
