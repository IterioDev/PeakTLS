...
QUIC. They consume and emit raw TLS Handshake bytes at QUIC encryption levels.
`SharpTls.Quic` separately ships `ITlsQuicDatagramTransport`, a datagram send/receive seam
with a direct-UDP implementation and a SOCKS5 UDP relay client (see
[SOCKS5-DATAGRAM-TRANSPORT.md](SOCKS5-DATAGRAM-TRANSPORT.md)); raw UDP is no longer purely
a surrounding-transport concern. The surrounding transport still owns packet numbers,
header and payload protection, CRYPTO frames, Retry, loss recovery, congestion control,
streams, connection IDs and HANDSHAKE_DONE. HTTP/3 and QPACK are not part of this library.
