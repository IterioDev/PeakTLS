...
### 17. Secure fingerprint randomization and profile rolling — REMOVED

Shipped, then deleted. Randomized profile generation and the origin-aware Roller both went:
this library sets every parameter deliberately, and a generated hello matches no real client
by construction. Reproduce a capture with `ClientHelloProfiles.Custom` instead.
...
### 30. QUIC datagram transport and SOCKS5 UDP relay

**Status: complete.** `ITlsQuicDatagramTransport` is the datagram seam the QUIC transport
is written against, so it never references `System.Net.Sockets.Socket` directly.
`TlsQuicUdpDatagramTransport` sends and receives directly over UDP; `TlsQuicSocks5Transport`
relays the same traffic through a SOCKS5 UDP association. Opt-in interoperability tests
against a real relay remain a phase-29-style external evidence gate.

Responsibility: a public datagram transport interface; a direct-UDP implementation; a
conforming SOCKS5 UDP ASSOCIATE client with RFC 1929 username/password authentication;
and explicit, documented accounting of the SOCKS5 encapsulation overhead against QUIC's
RFC 9000 §14.1 1200-byte minimum initial datagram. RFC 1928, RFC 1929, RFC 9000 §14.

Tests/gate: RFC-field-diagram wire vectors for every SOCKS5 message and both address
families, negative cases for every reply code/malformed field, an in-process deterministic
SOCKS5 relay covering association, authentication, wildcard `BND.ADDR` substitution,
control-connection termination and inbound-datagram discard rules, exact size-budget
assertions, a dedicated fuzz target for the reply and UDP header parsers, and opt-in
interoperability tests against a real relay (3proxy, Dante or gost), gated out of the
offline suite. See [SOCKS5-DATAGRAM-TRANSPORT.md](SOCKS5-DATAGRAM-TRANSPORT.md).
...
- QUIC packets, packet-number recovery, congestion control, streams, QPACK and HTTP/3
  (the recordless QUIC-TLS handshake adapter and the datagram transport/SOCKS5 relay
  in phase 30 are in scope);
